package com.youth4work.HSSC_2023.ui.views;
...
public class PrepButton extends CustomButtonFont implements View.OnTouchListener, HasOnViewAttachListener {
    ...
    public PrepButton(Context context) {
        super(context);
        init();
        this.setOnTouchListener(this);
    }

    public PrepButton(@NonNull Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    ...
    public PrepButton(@NonNull Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init() {
        ...
        mButtonColor = resources.getColor(R.color.prep_button_default_color);
        mShadowColor = resources.getColor(R.color.prep_button_default_shadow_color);
        mShadowHeight = resources.getDimensionPixelSize(R.dimen.prep_button_default_shadow_height);
        mCornerRadius = resources.getDimensionPixelSize(R.dimen.prep_button_default_conner_radius);
        setTransformationMethod(null);
    }

    private void parseAttrs(@NonNull Context context, AttributeSet attrs) {
        //Load from custom attributes
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.PrepButton);
        if (typedArray == null) return;
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.PrepButton_shadowEnabled) {
                isShadowEnabled = typedArray.getBoolean(attr, true); //Default is true
            } else if (attr == R.styleable.PrepButton_buttonColor) {
                mButtonColor = typedArray.getColor(attr, R.color.prep_button_default_color);
            } else if (attr == R.styleable.PrepButton_shadowColor) {
                mShadowColor = typedArray.getColor(attr, R.color.prep_button_default_shadow_color);
                isShadowColorDefined = true;
            } else if (attr == R.styleable.PrepButton_shadowHeight) {
                mShadowHeight = typedArray.getDimensionPixelSize(attr, R.dimen.prep_button_default_shadow_height);
            } else if (attr == R.styleable.PrepButton_cornerRadius) {
                mCornerRadius = typedArray.getDimensionPixelSize(attr, R.dimen.prep_button_default_conner_radius);
            }
        }
        typedArray.recycle();
    ...
    private void updateBackground(@Nullable Drawable background) {
